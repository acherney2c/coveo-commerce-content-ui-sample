import type {Cart, CartItemWithMetadata, CartState} from '@coveo/headless/commerce';

export function registerCartPersistence(cart: Cart): () => void {
  return cart.subscribe(() => {
    saveCartItemsToLocalStorage(cart.state);
  });
}

function saveCartItemsToLocalStorage(cartState: CartState): void {
  try {
